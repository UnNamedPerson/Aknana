...
if( isset($_POST['loginSubmit']) ){
     ...
     if( empty($Email) || empty($pwd) ){ // in case the email or passowrd fields were empty when the user clicked the login button
     ...
     // } 
          ...
          $sqlCommand = "SELECT * FROM userinfo WHERE userEmail=?;"; // the '*' in the sql command means 'all columns'
          ...
          if( !mysqli_stmt_prepare($sqliStatment, $sqlCommand) ){
               ...
               if( $resultRow = mysqli_fetch_assoc($result) ){
                    $pwdMatch = password_verify($pwd, $resultRow['userPWD']); // a variable that checks if the password entered by the user matches the one on the DB without having to hash the entered pwd
                    if($pwdMatch ==  true){ // start a session if the passwords match
                         session_start();
                         $_SESSION['uID'] = $resultRow['userId']; // sessions are a way to store user's info to be used on mult pages on the same website. We are storing the necessary info in the session array.
                         $_SESSION['uEmail'] = $resultRow['userEmail'];
